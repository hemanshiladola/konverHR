...
const EmployeeList = () => {
  ...
  // VirtualList renderItem function
  const renderEmployeeRow = useCallback(
    (item: Employee, _index: number, style: React.CSSProperties) => (
      <div style={style} key={item.EmpId} className="virtual-list-row">
  ...
  // Memoize the virtual list
  const virtualList = useMemo(
    () => (
      <VirtualList
        items={visibleData as unknown[]}
        itemHeight={64} // Approximate row height for employee rows
        containerHeight={600} // Visible area height
        renderItem={
          renderEmployeeRow as (
            item: unknown,
            index: number,
