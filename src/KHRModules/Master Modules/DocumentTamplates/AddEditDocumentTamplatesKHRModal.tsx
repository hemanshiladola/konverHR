        ...
        <div className="modal-content border-0">
          ...
          >
            ...
            <div className="d-md-none p-3">
              {/* Placeholders horizontal strip */}
              <div className="bg-white rounded border p-2 mb-3">
                <p className="fs-10 fw-black text-muted text-uppercase mb-2">Placeholders</p>
                <div className="d-flex gap-2 overflow-auto pb-1" style={{ flexWrap: "nowrap", scrollbarWidth: "none" }}>
                  {variables.map((v) => (
                    <button
                      key={v.value}
                      className="btn btn-outline-light text-dark border d-flex flex-column align-items-start p-2 flex-shrink-0"
                      style={{ minWidth: "100px" }}
                      onClick={() => injectVariable(v.value)}
                    >
                      <span className="fw-bold text-dark mb-1" style={{ fontSize: "11px" }}>{v.label}</span>
                      <code className="text-primary bg-light px-1 rounded" style={{ fontSize: "10px" }}>{v.value}</code>
                    </button>
                  ))}
                </div>
              </div>
              {/* Fields */}
              <div className="mb-2">
                <label className="fs-12 fw-bold text-muted">Internal Name</label>
                <input type="text" className="form-control fw-bold" value={name} onChange={(e) => handleTextChange(e, 50)} maxLength={50} />
              </div>
              <div className="mb-2">
              ...
              <button className="btn btn-dark w-100 fw-bold fs-12 mb-3" onClick={() => setIsSourceMode(!isSourceMode)}>
                <i className={`ti ${isSourceMode ? "ti-eye" : "ti-code"} me-2`}></i>
                {isSourceMode ? "Visual View" : "Edit HTML"}
              </button>
              <div className="card border-0 shadow-sm" style={{ minHeight: "300px" }}>
                {isSourceMode ? (
                  <textarea className="form-control border-0 p-3 fs-13" style={{ backgroundColor: "#1e1e1e", color: "#dcdcdc", fontFamily: "monospace", minHeight: "300px", resize: "vertical" }} value={content} onChange={(e) => setContent(e.target.value)} />
                ) : (
                  <ReactQuill ref={quillRef} theme="snow" value={content} onChange={setContent} style={{ minHeight: "300px" }} />
                )}
              </div>
            </div>
            ...
            <div className="d-none d-md-flex h-100">
              {/* Sidebar */}
              <div className="bg-white border-end p-3 d-flex flex-column flex-shrink-0" style={{ width: "280px" }}>
                <h6 className="fs-10 fw-black text-muted text-uppercase mb-3">Placeholders</h6>
                <div className="d-flex flex-column gap-2 overflow-auto mb-3" style={{ flex: 1, paddingRight: "4px" }}>
                  {variables.map((v) => (
                    <button key={v.value} className="btn btn-outline-light text-dark border d-flex justify-content-between align-items-center p-2" onClick={() => injectVariable(v.value)}>
                      <span className="fs-12 fw-bold text-start">{v.label}</span>
                      <code className="text-primary fs-11 bg-light px-1 rounded ms-2">{v.value}</code>
                    </button>
                  ))}
                </div>
                <button className="btn btn-dark w-100 fw-bold fs-12" onClick={() => setIsSourceMode(!isSourceMode)}>
                  <i className={`ti ${isSourceMode ? "ti-eye" : "ti-code"} me-2`}></i>
                  {isSourceMode ? "Visual View" : "Edit HTML"}
                </button>
              </div>
              ...
              <div className="flex-grow-1 p-4 overflow-auto">
                ...
                <div className="card border-0 shadow-sm h-100" style={{ minHeight: "500px" }}>
                  {isSourceMode ? (
                    <textarea className="form-control border-0 p-4 fs-13 h-100" style={{ backgroundColor: "#1e1e1e", color: "#dcdcdc", fontFamily: "monospace", minHeight: "500px" }} value={content} onChange={(e) => setContent(e.target.value)} />
                  ) : (
                    <ReactQuill ref={quillRef} theme="snow" value={content} onChange={setContent} style={{ height: "500px" }} />
                  )}
                </div>
              </div>
            </div>
