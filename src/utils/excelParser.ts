...
export const generateBulkUploadTemplate = async (masters: any) => {
  ...
  titleCell.fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: "FF004C8F" },
  };
  titleCell.alignment = { vertical: "middle", horizontal: "center" };

  // Subtitle
  instructionsSheet.getCell("B5").value =
    "📋 How to use this template properly:";
  instructionsSheet.getCell("B5").font = {
    bold: true,
    size: 14,
    color: { argb: "FFC00000" },
  };
  ...
  // Highlight warnings
  instructionsSheet.getCell(`B${rowIdx + 1}`).value =
    "⚠️ CRITICAL: Ensure you do NOT leave empty blank rows between employees.";
  instructionsSheet.getCell(`B${rowIdx + 1}`).font = {
    bold: true,
    size: 12,
    color: { argb: "FFFF0000" },
  };
  ...
  const getFirst = (list: any[]) =>
    list && list.length > 0 ? list[0].label : "";

  // Extract purely labels
  const depts = (masters.departments || [])
    .map((d: any) => d.label)
    .filter(Boolean);
  const desigs = (masters.designations || [])
    .map((d: any) => d.label)
    .filter(Boolean);
  const branches = (masters.branches || [])
    .map((d: any) => d.label)
    .filter(Boolean);
  const policies = (masters.attendancePolicies || [])
    .map((d: any) => d.label)
    .filter(Boolean);
  const schedules = (masters.workingSchedules || [])
    .map((d: any) => d.label)
    .filter(Boolean);
  const rosters = (masters.shiftRosters || [])
    .map((d: any) => d.label)
    .filter(Boolean);
  const countries = (masters.countries || [])
    .map((d: any) => d.label)
    .filter(Boolean);
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const sheetName =
          workbook.SheetNames.find((n) => n.includes("Employee_Data")) ||
        ...
        const validRows = jsonData.filter(
          (r: any) => r["Employee Name *"] && r["Department *"],
        );
        resolve(validRows);
      } catch (error) {
        reject(error);
...
export const mapExcelRowToPayload = (row: any, masters: any) => {
  ...
  return {
    ...
    attendance_policy_id: findId(
      masters.attendancePolicies,
      row["Attendance Policy *"],
    ),

    resource_calendar_id: findId(
      masters.workingSchedules,
      row["Working Schedule (Resource Calendar)"],
    ),
    ...
    probation_period: isNaN(Number(row["Probation Period (Months)"]))
      ? 6
      : Number(row["Probation Period (Months)"]),
    ...
    marital: (
      row["Marital Status (single/married/divorced/widowed)"] || "single"
    ).toLowerCase(),
    ...
    emergency_contact_mobile: row["Emergency Contact Mobile"]
      ? String(row["Emergency Contact Mobile"])
      : "",
    ...
    bank_swift_code: row["Bank SWIFT Code"]
      ? String(row["Bank SWIFT Code"])
      : "",
    currency_id: row["Currency (Default: INR)"] || "INR",

    employment_type: (
      row["Employment Type (permanent/contract/intern/probation)"] ||
      "permanent"
    ).toLowerCase(),
    employee_password: row["Employee Password"]
      ? String(row["Employee Password"])
      : "",
